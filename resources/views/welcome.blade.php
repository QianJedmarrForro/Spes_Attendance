<x-layout>
    ...
    <div class="bg-white/90 backdrop-blur-md p-10 rounded-2xl shadow-xl text-center max-w-md w-full border border-blue-100 relative">
        ...
        <!-- QR Code Claim Form -->
        <form action="{{ route('students.store') }}" method="POST" class="space-y-4 text-left">
            @csrf
            <div>
        ...
        <div class="relative flex py-4 items-center">
            ...
            <span class="flex-shrink mx-4 text-gray-400 text-xs font-semibold uppercase tracking-wider">Management</span>
            <div class="flex-grow border-t border-gray-100"></div>
        </div>
        ...
        <div class="space-y-2">
            ...
            <a href="{{ route('terminal.scanner') }}" 
               class="inline-flex items-center justify-center gap-1.5 w-full bg-blue-50 hover:bg-blue-100 text-blue-600 text-sm font-semibold py-2.5 rounded-xl transition duration-200 border border-blue-100/50">
                <svg xmlns="http://w3.org" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v1m6 11h2m-6 0h-2v4m0-11v3m0 0h.01M12 12h.01M16 12h.01M21 12a9 9 0 11-18 0 a9 9 0 0118 0z" />
                </svg>
                Scan Attendance QR
            </a>
